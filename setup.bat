...
powershell "$s=(New-Object -COM WScript.Shell).CreateShortcut('Partlocater.lnk');$p=(Get-Item -Path '.\').FullName;$s.TargetPath='pythonw';$s.Arguments='partlocater.pyw';$s.WorkingDirectory=Join-Path $p 'scripts';$s.IconLocation=Join-Path (Join-Path $p 'assets') 'pl.ico';$s.Save()"
